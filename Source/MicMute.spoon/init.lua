...
function obj:updateMicMute(muted)
  if muted == -1 then muted = hs.audiodevice.defaultInputDevice():muted() end
  if muted then
...
function obj:toggleMicMute()
  ...
  if mic:muted() then
    ...
    if zoom then
      ...
      if not ok then
        hs.timer.doAfter(0.5, function() zoom:selectMenuItem 'Mute Audio' end)
      end
    end
  end
  obj:updateMicMute(-1)
...
function obj:bindHotkeys(mapping, latch_timeout)
  ...
  if latch_timeout then
    ...
    self.hotkey = hs.hotkey.bind(mods, key, function() self:toggleMicMute() end)
  end
...
function obj:init()
  ...
  obj.mute_menu:setClickCallback(function() obj:toggleMicMute() end)
  obj:updateMicMute(-1)

  hs.audiodevice.watcher.setCallback(function(arg)
    if string.find(arg, "dIn ") then obj:updateMicMute(-1) end
  end)
